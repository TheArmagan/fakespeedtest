...
app.once("ready", () => {
    ...
    ipcMain.on("fakeTest", async (event, opts) => {
        ...
        let response = await axios({
            ...
            data: Object.entries({
                ...
                accuracy: 10,
                hash: MD5(`${input.ping}-${input.upload}-${input.download}-297aae72`).toString(enc.Hex),
                promo: ""
            }).map(i => `${i[0]}=${i[1]}`).join("&")
        })
    ...
    ipcMain.on("serverList", async () => {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            let parsed = await XML.parseStringPromise(xmlData, { normalize: true, normalizeTags: true });
